...
yolo_openimages.py
==================
Script completo: baixa subconjunto do Open Images v7, converte para
formato YOLO e treina / roda inferência com YOLOv8.

Requisitos:
    pip install ultralytics fiftyone opencv-python pyyaml

Uso — treinar:
    python yolo_openimages.py train --classes Car Person Bicycle --samples 300

Uso — inferência em imagem/vídeo/webcam:
    python yolo_openimages.py infer --weights runs/train/best.pt --source foto.jpg
    python yolo_openimages.py infer --weights best.pt --source video.mp4
    python yolo_openimages.py infer --weights best.pt --source 0
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# DETECÇÃO AUTOMÁTICA DE DEVICE  (GPU ou CPU)
# ─────────────────────────────────────────────────────────────────────────────

def auto_device(requested: str) -> str:
    """
    Detecta automaticamente se CUDA está disponível.
    - Se o usuário pediu 'cpu'  → usa CPU
    - Se o usuário pediu 'auto' → testa CUDA; cai para CPU se não houver
    - Se o usuário pediu '0'    → testa CUDA; cai para CPU se não houver
    Retorna a string de device correta para passar ao YOLO.
    """
    import torch
    ...
    if torch.cuda.is_available():
        ...
        print(
            f"      Device  : GPU '{nome}' ({n} disponível{'is' if n > 1 else ''})")
...
# ─────────────────────────────────────────────────────────────────────────────
# 1. DOWNLOAD + CONVERSÃO  (Open Images → formato YOLO)
# ─────────────────────────────────────────────────────────────────────────────
...
def download_and_convert(
    ...
    """
    Baixa amostras do Open Images via FiftyOne e converte para formato YOLO.

    Formato YOLO por linha no .txt:
        <class_id> <cx> <cy> <width> <height>   (todos normalizados 0-1)

    Retorna o caminho do dataset.yaml gerado.
    """
    import fiftyone.zoo as foz

    print(f"\n[1/3] Baixando Open Images v7")
    ...
    dataset = foz.load_zoo_dataset(
        ...
        classes=classes,
        max_samples=samples,
        dataset_name=f"oi_{'_'.join(classes[:3])}_{samples}",
        overwrite=True,
    )

    # Mapeamento classe → id (ordem alfabética)
...
def train(
    ...
    """Treina o modelo YOLO e retorna o caminho dos melhores pesos."""
    ...
    # baixa pesos pré-treinados se necessário
    model = YOLO(str(SCRIPT_DIR / f"{model_name}.pt"))
    ...
    # Salvar resultado do treino em arquivo .txt
    txt_path = RESULT_DIR / "resultado_treino.txt"
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("=== RESULTADO DO TREINAMENTO ===\n")
        ...
        f.write(f"Melhores pesos (best.pt): {best.resolve()}\n")
...
def infer(weights: str, source: str, conf: float, save: bool) -> None:
    ...
    model = YOLO(weights)
    # Configurar pasta de salvamento da inferência dentro de RUNS_DIR
    ...
    # Coletar resultados para exibir e salvar em arquivo .txt
    txt_path = RESULT_DIR / "resultado_inferencia.txt"
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("=== RESULTADO DA INFERÊNCIA ===\n")
    ...
    if save:
        ...
        print(f"\n  [OK] Resultado salvo em: {save_dir}")
...
def validate(weights: str, data_yaml: str) -> None:
    ...
    # Salvar resultado do teste em arquivo .txt
    txt_path = RESULT_DIR / "resultado_validacao.txt"
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("=== RESULTADO DA VALIDAÇÃO ===\n")
        f.write(f"Pesos utilizados: {Path(weights).resolve()}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="YOLO + Open Images v7 — pipeline completo",
        formatter_class=argparse.RawTextHelpFormatter,
    )
    sub = parser.add_subparsers(dest="cmd", required=True)

    # ── train ────────────────────────────────────────────────────────────
    p_train = sub.add_parser("train", help="Baixar Open Images e treinar YOLO")
    p_train.add_argument(
        "--classes", nargs="+", required=True,
        help="Classes do Open Images (ex: Car Person Bicycle)\n"
             "Lista completa: https://storage.googleapis.com/openimages/v5/class-descriptions-boxable.csv"
    )
    ...
    p_train.add_argument("--split", default="validation",
                         ...
                         help="Split do Open Images (default: validation)")
    ...
    p_train.add_argument("--device", default="auto",
                         help="'auto' detecta GPU/CPU automaticamente (default)\n"
                              "'cpu' força CPU | '0' força GPU 0")
    p_train.add_argument("--out-dir", default="datasets/openimages_yolo",
                         help="Pasta de saída do dataset convertido")

    # ── infer ────────────────────────────────────────────────────────────
    p_infer = sub.add_parser(
        "infer", help="Rodar inferência com modelo treinado")
    ...
    p_infer.add_argument("--save",    action="store_true",
                         help="Salvar resultado ao invés de exibir")
