...
def YOLOv3(input_layer, NUM_CLASS):
    ...
    conv = keras.ops.concatenate([conv, route_2], axis=-1)
    ...
    conv = keras.ops.concatenate([conv, route_1], axis=-1)
...
def YOLOv4(input_layer, NUM_CLASS):
    ...
    conv = keras.ops.concatenate([route_2, conv], axis=-1)
    ...
    conv = keras.ops.concatenate([route_1, conv], axis=-1)
    ...
    conv = keras.ops.concatenate([conv, route], axis=-1)
...
def decode_train(conv_output, output_size, NUM_CLASS, STRIDES, ANCHORS, i=0, XYSCALE=[1, 1, 1]):
    ...
    pred_xywh = keras.ops.concatenate([pred_xy, pred_wh], axis=-1)

    pred_conf = tf.sigmoid(conv_raw_conf)
    pred_prob = tf.sigmoid(conv_raw_prob)

    return keras.ops.concatenate([pred_xywh, pred_conf, pred_prob], axis=-1)
...
def decode_tflite(conv_output, output_size, NUM_CLASS, STRIDES, ANCHORS, i=0, XYSCALE=[1,1,1]):
    ...
    pred_prob = keras.ops.concatenate(conv_raw_score, axis=1)
    ...
    pred_wh = keras.ops.concatenate(conv_raw_dwdh, axis=1)
    ...
    pred_xy = keras.ops.concatenate(conv_raw_dxdy, axis=1)
    pred_xywh = keras.ops.concatenate([pred_xy, pred_wh], axis=-1)
    return pred_xywh, pred_prob
    # return keras.ops.concatenate([pred_xywh, pred_conf, pred_prob], axis=-1)
...
def filter_boxes(box_xywh, scores, score_threshold=0.4, input_shape = tf.constant([416,416])):
    ...
    # return keras.ops.concatenate([boxes, pred_conf], axis=-1)
