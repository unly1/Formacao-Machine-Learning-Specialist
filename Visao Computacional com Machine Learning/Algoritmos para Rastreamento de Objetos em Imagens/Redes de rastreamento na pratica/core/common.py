...
def convolutional(input_layer, filters_shape, downsample=False, activate=True, bn=True, activate_type='leaky'):
    ...
    if activate == True:
        if activate_type == "leaky":
            ...
            conv = keras.layers.Activation('mish')(conv)
    return conv

def mish(x):
    return keras.layers.Activation('mish')(x)
...
def upsample(input_layer):
    return keras.ops.image.resize(input_layer, (input_layer.shape[1] * 2, input_layer.shape[2] * 2), interpolation='bilinear')
