...
def cspdarknet53(input_data):
    ...
    input_data = keras.ops.concatenate([input_data, route], axis=-1)
    ...
    input_data = keras.ops.concatenate([keras.layers.MaxPool2D(pool_size=13, strides=1, padding='same')(input_data), keras.layers.MaxPool2D(pool_size=9, strides=1, padding='same')(input_data), keras.layers.MaxPool2D(pool_size=5, strides=1, padding='same')(input_data), input_data], axis=-1)
...
def cspdarknet53_tiny(input_data):
    ...
    route_1 = input_data
    input_data = common.convolutional(input_data, (3, 3, 32, 32))
    input_data = keras.ops.concatenate([input_data, route_1], axis=-1)
    input_data = common.convolutional(input_data, (1, 1, 32, 64))
    input_data = keras.ops.concatenate([route, input_data], axis=-1)
