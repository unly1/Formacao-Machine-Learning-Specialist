# YOLOv4 + DeepSORT — Rastreamento de Objetos na Prática

> **Repositório original:** [theAIGuysCode/yolov4-deepsort](https://github.com/theaiguyscode/yolov4-deepsort)
...
Este módulo é parte da formação **Visão Computacional com Machine Learning** e demonstra na prática o uso combinado de:

| Tecnologia | Papel no pipeline |
| :--- | :--- |
| **YOLOv4** | Detecção de objetos por frame (Who / What) |
| **DeepSORT** | Rastreamento multi-objeto em tempo real (Who over Time) |
| **TensorFlow 2.x / Keras** | Backend de inferência e exportação do modelo |

O projeto parte do código do repositório oficial [theAIGuysCode/yolov4-deepsort](https://github.com/theaiguyscode/yolov4-deepsort) (⭐ 1.4k · 🍴 750 forks), que por sua vez integra dois trabalhos de referência da literatura:

- [`hunglc007/tensorflow-yolov4-tflite`](https://github.com/hunglc007/tensorflow-yolov4-tflite) — backbone YOLOv4 em TensorFlow
- [`nwojke/deep_sort`](https://github.com/nwojke/deep_sort) — algoritmo DeepSORT original
...
```
Redes de rastreamento na pratica/
├── core/                  # Camadas YOLOv4 (common.py, yolov4.py, utils.py)
├── deep_sort/             # Algoritmo DeepSORT (detection.py, preprocessing.py, ...)
├── data/                  # Vídeos de entrada e pesos yolov4.weights
├── checkpoints/           # Modelo TensorFlow exportado (gerado pelo save_model.py)
├── outputs/               # Vídeo de saída com rastreamento (result.avi)
├── model_data/            # Labels e metadados do modelo
├── tools/                 # Utilitários auxiliares (generate_detections.py)
├── save_model.py          # Converte .weights → SavedModel TensorFlow
├── object_tracker.py      # Script de rastreamento (original do repo)
├── yolov4_deepsort.py     # Script unificado adaptado para Windows local
├── requirements.txt       # Dependências CPU
└── requirements-gpu.txt   # Dependências GPU
```

---

# YOLOv4 DeepSORT: Guia de Migração (Google Colab → Local Windows)
